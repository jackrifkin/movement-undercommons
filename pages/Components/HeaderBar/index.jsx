...
export default function HeaderBar({
    ...
    const [currentUnderlineColor, setCurrentUnderlineColor] = useState(underlineColors[0]);
    ...
    return (
        <div
            ...
            style={{ ...{ borderColor: currentUnderlineColor, backgroundColor: currentBackgroundColor }, ...(alignTextLeft ? alignLeftDivStyle : alignRightDivStyle) }}
            onMouseEnter={handleOnMouseEnter}
            onMouseLeave={handleOnMouseLeave}
        >
            <h1 className={`${styles.headerBarText} abolitionRegular`} style={{ ...{color: currentTextColor, transition: "color 0.5s ease" }, ...(alignTextLeft ? alignLeftTextStyle : alignRightTextStyle)}}>{text}</h1>
        </div>
    )
}