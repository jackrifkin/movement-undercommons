...
export default function HorizontalGallery({items, hasShadows, defaultItemWidth, itemHeightFactor = 1}) {
    ...
    return (
      <div className={styles.galleryContainer}>
        {canScroll && 
        <button className={`${styles.galleryButton} left`} onClick={() => scroll('left')} style={{paddingRight: '15px'}}>
          <Image src={'/chevronLeft.png'} width={25} height={75}/>
        </button>}
        ...
        {canScroll && 
        <button className={`${styles.galleryButton} right`} onClick={() => scroll('right')} style={{paddingLeft: '15px'}}>
            <Image src={'/chevronRight.png'} width={25} height={75}/>
        </button>}
      </div>
    );
}